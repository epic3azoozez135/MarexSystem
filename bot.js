 ...
 **
__ Marex System __ By: - sR , عــزووووز#4888
...
m.delete(1500);
    ...
    .setTitle('New Member')
    .setDescription('Welcome To Starex , Have a good Time :heart: :rose: ')
    .addField('**ID Member:',"" +  member.user.id, true)
    .addField('**Tag Member:', member.user.discriminator, true)
...
const adminprefix = "$";
const devs = ['510829335519821825','511491524077420555'];
client.on('message', message => {
...
client.on("guildMemberAdd", member => {
  ...
  const channel = sWlc[member.guild.id].channel
    ...
    let welcomer = member.guild.channels.find('marex', sChannel);
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("526669622951280651")
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.find('name', "marex");
    if (!channel) {
    ...
    var guild;
    while (!guild)
        guild = client.guilds.get("526664748759253022")
    guild.fetchInvites().then((data) => {
        data.forEach((Invite, key, map) => {
            var Inv = Invite.code;
            if (dat[Inv])
